...
const AssetClassDialog: React.FC<AssetClassDialogProps> = ({
  open,
  symbol,
  onSave,
  onCancel,
  ...
  const loadAssetStyles = async () => {
    ...
    try {
      ...
      // Select the "Default" style by default if it exists
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'Failed to save asset classification');
    } finally {
      setLoading(false);
    }
  };
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-white dark:bg-gray-800 rounded-lg shadow-xl w-[calc(100%-2rem)] sm:w-full max-w-md max-h-[90vh] overflow-y-auto data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 z-50">
          <div className="p-4 sm:p-6">
            ...
            <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 mb-4 sm:mb-6">
              This is the first transaction for <span className="font-semibold text-gray-900 dark:text-white">{symbol}</span>. 
              Please classify this asset to help organize your portfolio.
            </p>
...
export default AssetClassDialog;
